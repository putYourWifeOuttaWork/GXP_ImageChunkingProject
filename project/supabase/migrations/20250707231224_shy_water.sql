/*
  # Add NULL Value Handling for Report Filters
  
  1. New Features
    - Add special handling for "NULL" string values in report filters
    - Convert "NULL" string to SQL NULL with proper IS NULL / IS NOT NULL syntax
    - Add validation for other operators with NULL values
    - Improve metric expression handling
    
  2. Purpose
    - Fix "invalid input syntax for type numeric: NULL" error
    - Support filtering for NULL/non-NULL values in reports
    - Make report builder more user-friendly
*/

-- Update the execute_custom_report_query function with proper NULL value handling
...
BEGIN
  ...
  ELSE
    ...
    ELSE
      ...
      metric_expression := format('%s(%I.%I)', metric_function, main_entity, metric_field);
    END IF;
  ...
  IF p_report_configuration -> 'filters' IS NOT NULL AND jsonb_array_length(p_report_configuration -> 'filters') > 0 THEN
    ...
    FOR i IN 0..jsonb_array_length(p_report_configuration -> 'filters') - 1 LOOP
      ...
      -- Check if the filter value is "NULL" (case insensitive)
      IF lower(filter_value) = 'null' THEN
        -- Handle NULL values with IS NULL / IS NOT NULL
        CASE filter_operator
          WHEN '=' THEN
            filter_conditions := filter_conditions || format('%I.%I IS NULL', main_entity, filter_field);
          WHEN '!=' THEN
            filter_conditions := filter_conditions || format('%I.%I IS NOT NULL', main_entity, filter_field);
          ELSE
            -- Other operators don't work with NULL
            RAISE EXCEPTION 'Operator % cannot be used with NULL values', filter_operator;
        END CASE;
      ELSE
        -- Handle normal non-NULL values
        CASE filter_operator
          WHEN '=' THEN 
...
COMMENT ON FUNCTION execute_custom_report_query IS 'Executes a dynamic query based on the provided report configuration with improved NULL value handling';