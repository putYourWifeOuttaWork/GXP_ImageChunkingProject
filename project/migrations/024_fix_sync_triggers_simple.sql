-- Simple, robust fix for all sync trigger column issues
-- This version explicitly lists only columns that exist in source tables
...
-- Create new sync function with explicit column list
CREATE OR REPLACE FUNCTION sync_gasifier_to_partitioned()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'INSERT' THEN
        -- Insert only columns that exist in source table
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        -- Insert only columns that exist in source table
        -- Excluding: split_image_status, daysinthisprogramphase, todays_day_of_phase, 
        -- yesterday_growth_index, trend_petri_velocity, experiment_role
...
-- ========================================
-- RECREATE TRIGGERS ON SOURCE TABLES ONLY
-- ========================================
...
-- ========================================
-- CLEAN UP TRIGGERS ON PARTITIONED TABLES
-- ========================================

-- First, drop the problematic triggers on partition children
DO $$
DECLARE
    r RECORD;
BEGIN
    -- Find and drop all sync triggers on partition tables
    FOR r IN 
        SELECT DISTINCT nspname, relname, tgname
        FROM pg_trigger t
        JOIN pg_class c ON t.tgrelid = c.oid
        JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE t.tgname LIKE 'sync_%_observations_trigger'
        AND (c.relname LIKE 'petri_obs_%' OR c.relname LIKE 'gasifier_obs_%')
        AND c.relname NOT IN ('petri_observations', 'gasifier_observations')
    LOOP
        BEGIN
            EXECUTE format('DROP TRIGGER %I ON %I.%I CASCADE', r.tgname, r.nspname, r.relname);
            RAISE NOTICE 'Dropped trigger % from %.%', r.tgname, r.nspname, r.relname;
        EXCEPTION
            WHEN undefined_object THEN
                RAISE NOTICE 'Trigger % on %.% already dropped', r.tgname, r.nspname, r.relname;
            WHEN OTHERS THEN
                RAISE NOTICE 'Could not drop trigger % on %.%: %', r.tgname, r.nspname, r.relname, SQLERRM;
        END;
    END LOOP;
END $$;
...
-- Show column differences for reference
SELECT 
    'Columns in gasifier_observations_partitioned but NOT in gasifier_observations:' as info
UNION ALL
...
SELECT ''
UNION ALL
SELECT 'Columns in petri_observations_partitioned but NOT in petri_observations:' as info
UNION ALL
SELECT string_agg(column_name, ', ')
FROM (
    SELECT column_name 
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '✅ ALL sync triggers have been fixed!';
    RAISE NOTICE '📝 The following columns are excluded from sync:';
    RAISE NOTICE '   - Gasifier: forecasted_expiration, trend_gasifier_velocity, trend';
    RAISE NOTICE '   - Petri: split_image_status, daysinthisprogramphase, todays_day_of_phase,';
    RAISE NOTICE '            yesterday_growth_index, trend_petri_velocity, experiment_role';
    RAISE NOTICE '🚀 These will be calculated by triggers on the partitioned tables.';
    RAISE NOTICE '✨ Submission creation should now work without ANY column errors!';
END $$;