/*
  # Update Create Submission Session Function for Image Splitting Support
  
  1. Updates
    - Modify the create_submission_session RPC function to handle split image templates
    - For petri templates marked with is_split_image_template=true, create two observation records
    - Assign split_codes to the created petri observations
    
  2. Purpose
    - Support the creation of linked petri observations for split images
    - Maintain consistent data between the two halves of split observations
*/

-- Drop the existing function to recreate it with updated logic
    ...
    LOOP
      ...
      IF (v_petri_template->>'is_split_image_template')::BOOLEAN IS TRUE THEN
        ...
        VALUES (
          ...
          p_site_id,
          v_left_code,
          v_petri_template->>'plant_type',
          v_petri_template->>'fungicide_used',
          v_petri_template->>'surrounding_water_schedule',
          v_petri_template->>'placement',
          v_petri_template->>'placement_dynamics',
  ...
  IF p_gasifier_templates IS NOT NULL AND jsonb_array_length(p_gasifier_templates) > 0 THEN
    ...
    LOOP
      ...
      VALUES (
        ...
        v_gasifier_template->>'chemical_type',
        v_gasifier_template->>'placement_height',
        v_gasifier_template->>'directional_placement',
        v_gasifier_template->>'placement_strategy',
...
COMMENT ON FUNCTION public.create_submission_session IS 'Creates a submission and session with optional petri and gasifier observations from templates. Supports split image petri templates.';