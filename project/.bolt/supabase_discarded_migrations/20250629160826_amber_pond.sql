-- Further fixes for create_submission_session function to avoid enum casting errors

-- Drop the existing function 
DROP FUNCTION IF EXISTS public.create_submission_session(UUID, UUID, JSONB, JSONB, JSONB);

-- Re-create with proper handling of null values in template data
...
COMMENT ON FUNCTION public.create_submission_session IS 'Creates a submission and session with optional petri and gasifier observations from templates. Handles null values for enum fields by providing defaults.';