...
-- Create a new version of the function with mandatory parameters first, then optional parameters
-- This follows PostgreSQL's requirement that parameters with defaults must come after those without defaults
...
COMMENT ON FUNCTION public.create_submission_session IS 'Creates a submission and session with optional petri and gasifier observations from templates. Parameters are ordered with required parameters first, then optional parameters with defaults.';

-- Add an explicit trigger for the split processing
CREATE OR REPLACE TRIGGER trigger_split_petri_observation
AFTER INSERT OR UPDATE ON public.petri_observations
FOR EACH ROW
...
COMMENT ON TRIGGER trigger_split_petri_observation ON public.petri_observations IS 'Triggers the split_petri_observation edge function when a split image is uploaded to process the splitting';